...
# ---------------------------------------------------------------------------
# Image text translation (OCR + overlay)
# ---------------------------------------------------------------------------

_TESSERACT_LANG_MAP = {  # our lang code -> tesseract language pack code
    "zh-tw": "chi_tra", "zh-cn": "chi_sim", "en": "eng", "ja": "jpn", "ko": "kor",
    "fr": "fra", "de": "deu", "es": "spa", "vi": "vie", "th": "tha",
    "id": "ind", "ru": "rus", "ar": "ara", "pl": "pol",
}
_FC_LANG_MAP = {  # our lang code -> fontconfig language tag
    "zh-tw": "zh-tw", "zh-cn": "zh-cn", "en": "en", "ja": "ja", "ko": "ko",
    "fr": "fr", "de": "de", "es": "es", "vi": "vi", "th": "th",
    "id": "id", "ru": "ru", "ar": "ar", "pl": "pl",
}


@lru_cache(maxsize=32)
def _font_path_for(lang_code: str) -> str | None:
    """Resolve a font file covering the given language via fontconfig (fc-match)."""
    fc_lang = _FC_LANG_MAP.get(lang_code.lower(), "en")
    try:
        out = subprocess.run(
            ["fc-match", "-f", "%{file}", f":lang={fc_lang}"],
...
def _ocr_lines(img: Image.Image, source_lang: str) -> list[dict]:
    """Run OCR and group detected words into lines with merged bounding boxes."""
    tess_lang = _TESSERACT_LANG_MAP.get(source_lang.lower(), "eng")
    ...
    return [
        {"text": " ".join(l["words"]), "box": (l["x0"], l["y0"], l["x1"] - l["x0"], l["y1"] - l["y0"])}
        for l in lines.values()
    ]


def _edge_avg_color(img: Image.Image, x: int, y: int, w: int, h: int) -> tuple[int, int, int]:
    """Approximate the background color by sampling pixels just outside the text box."""
    px = img.load()
    iw, ih = img.size
    samples = []
    for sx, sy in ((x - 2, y), (x + w + 2, y), (x, y - 2), (x, y + h + 2)):
...
def _render_overlay(img: Image.Image, lines: list[dict], translated_lines: list[str], target_lang: str) -> bytes:
    """Cover each detected text line with its background color and draw the translation on top."""
    img = img.convert("RGB")
    draw = ImageDraw.Draw(img)
    font_path = _font_path_for(target_lang)
    for line, text in zip(lines, translated_lines):
        text = text.strip()
        if not text:
            continue
        x, y, w, h = line["box"]
        bg = _edge_avg_color(img, x, y, w, h)
        draw.rectangle([x, y, x + w, y + h], fill=bg)
        fg = (0, 0, 0) if sum(bg) > 380 else (255, 255, 255)
        size = max(10, int(h * 0.8))
...
def _translate_image_overlay(
    data: bytes, source_lang: str, target_lang: str,
    glossary: dict | None, substitutions: dict | None,
) -> bytes | None:
    """Run OCR on image bytes, translate detected lines, and draw the translation back onto the image.

    Returns the new image bytes, or None if OCR found nothing or the translated
    line count doesn't match (overlay would be misaligned — caller should fall back).
    """
    try:
        img = Image.open(io.BytesIO(data))
        lines = _ocr_lines(img, source_lang)
        if not lines:
            return None
        joined = "\n".join(l["text"] for l in lines)
        translated = translate_text(joined, source_lang, target_lang, glossary, substitutions)
        if not translated:
            return None
        translated_lines = translated.splitlines()
        if len(translated_lines) != len(lines):
            return None
        return _render_overlay(img, lines, translated_lines, target_lang)
    except Exception as e:
        print(f"[image-translate] overlay error ({source_lang}->{target_lang}): {e}")
        return None


def _ocr_text(data: bytes, source_lang: str) -> str | None:
    """Run OCR on image bytes and return the plain extracted text (used for fallback translation)."""
    try:
        img = Image.open(io.BytesIO(data))
        lines = _ocr_lines(img, source_lang)
        if not lines:
...
@bot.tree.context_menu(name="翻譯圖片文字")
async def translate_image_context_menu(interaction: discord.Interaction, message: discord.Message):
    await interaction.response.defer(ephemeral=True)
    uid = interaction.user.id
    ...
    if isinstance(message.channel, discord.Thread) and message.channel.parent_id:
        ch_id = message.channel.parent_id
    if ch_id not in all_gc:
        await interaction.followup.send(await _ui_msg(uid, "此頻道不是語言頻道。"), ephemeral=True)
        return

    image_att = next((a for a in message.attachments if (a.content_type or "").startswith("image/")), None)
    if image_att is None:
        await interaction.followup.send(await _ui_msg(uid, "此訊息沒有圖片附件。"), ephemeral=True)
        return

    async with aiohttp.ClientSession() as dl:
        async with dl.get(image_att.url) as resp:
            if resp.status != 200:
                await interaction.followup.send(await _ui_msg(uid, "圖片下載失敗。"), ephemeral=True)
                return
            data = await resp.read()

    source_lang = all_gc[ch_id]["lang"]
    guild_channels = _group_channels(all_gc, ch_id)
    targets = [(cid, info) for cid, info in guild_channels.items()
               if cid != ch_id and info.get("webhook_url")]
    if not targets:
        await interaction.followup.send(await _ui_msg(uid, "此群組沒有其他語言頻道。"), ephemeral=True)
        return

    guild_glossary = get_guild_glossary(interaction.guild_id, _glossary_data)
    guild_substitutions = get_guild_substitutions(interaction.guild_id, _substitutions_data)
    cluster = _msg_clusters.get(message.id)
    username = message.author.display_name
    avatar_url = str(message.author.display_avatar.url)
    ...
    fallback_count = 0
    async with aiohttp.ClientSession() as session:
        for target_ch_id, info in targets:
            target_lang = info["lang"]
            webhook_url = info["webhook_url"]
            thread_id = cluster.get("thread_channels", {}).get(target_ch_id) if cluster else None
            link = _ref_msg_link(cluster, target_ch_id, interaction.guild_id) if cluster else None

            overlay = await asyncio.to_thread(
                _translate_image_overlay, data, source_lang, target_lang, guild_glossary, guild_substitutions
            )
            try:
                webhook = discord.Webhook.from_url(webhook_url, session=session)
                if overlay is not None:
                    send_kwargs: dict = {
                        "username": username, "avatar_url": avatar_url, "wait": True,
                        "files": [discord.File(io.BytesIO(overlay), filename=f"translated_{target_lang}.png")],
                    }
                    if link:
                        send_kwargs["content"] = f"[↗]({link})"
                    if thread_id:
                        send_kwargs["thread"] = discord.Object(id=thread_id)
                    ...
                    if ocr_text is None:
                        ocr_text = await asyncio.to_thread(_ocr_text, data, source_lang) or ""
                    if not ocr_text:
                        continue
                    translated = await asyncio.to_thread(
                        translate_text, ocr_text, source_lang, target_lang, guild_glossary, guild_substitutions
                    )
                    if not translated:
                        continue
                    content = f"🖼️ {translated}" + (f"  [↗]({link})" if link else "")
                    send_kwargs = {"username": username, "avatar_url": avatar_url, "content": content, "wait": True}
                    if thread_id:
                        send_kwargs["thread"] = discord.Object(id=thread_id)
                    await webhook.send(**send_kwargs)
                    fallback_count += 1
            except Exception as e:
                print(f"[image-translate] send failed ({target_ch_id}): {e}")

    if overlay_count == 0 and fallback_count == 0:
        await interaction.followup.send(await _ui_msg(uid, "圖片中未偵測到文字，或翻譯失敗。"), ephemeral=True)
        return

    summary = await _ui_msg(uid, f"已產生 {overlay_count} 張覆蓋翻譯圖、{fallback_count} 則文字翻譯。")
    if ocr_text:
        ocr_label = await _ui_msg(uid, "辨識原文")
        summary += f"\n\n{ocr_label}：\n{ocr_text}"
