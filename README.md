...
**① 專有名詞（不翻譯）**：人名、地名、品牌名等，在所有語言頻道直接保留原文。
```
/addproper word:Jim
/addproper word:Starlight Guild
```
之後訊息中出現 `Jim` 時，無論翻譯到哪個語言，皆顯示 `Jim`，不會被翻譯成「吉姆」。

**② 翻譯詞彙**：為特定詞彙指定固定翻譯，避免機器翻譯出錯。
```
...
| 功能 | 說明 |
|------|------|
| 詞彙表優先 | 詞彙表詞彙完全不經翻譯 API，直接套用設定翻譯，確保術語準確且不耗費 API 呼叫 |
| 來源語言優先策略 | 先以頻道設定的語言代碼翻譯，若失敗再以 `source=auto` 自動偵測重試，對 CJK 語言（中日韓）更穩定 |
| 翻譯快取 | 相同內容不重複呼叫 API，節省資源 |
| 重試機制 | 遇到 Google Translate 限流（429）或回傳原文未翻譯時，自動退避重試最多 4 次（1 秒、2 秒、4 秒） |
| 多行訊息逐行翻譯 | 多行訊息拆成每行分別翻譯再合併，避免 Google API 只翻譯第一行的已知問題 |
