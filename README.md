...
右鍵翻譯功能依據你設定的語言清單進行翻譯，**所有人皆可設定，無需管理員權限**。

```
/addmylang lang:zh-TW   ← 新增繁體中文
/addmylang lang:en      ← 新增英文
/addmylang lang:ja      ← 新增日文
```

| Slash 指令 | 所需權限 | 說明 |
|-----------|---------|------|
| `/addmylang lang:<代碼>` | 所有人 | 新增個人翻譯語言（可重複執行加入多個） |
| `/removemylang lang:<代碼>` | 所有人 | 移除個人翻譯語言 |
| `/listmylang` | 所有人 | 查看目前設定的語言清單 |

設定完成後，右鍵任意訊息 → **Apps**，可使用以下三個個人指令：

| 指令 | 說明 |
|------|------|
| **翻譯此訊息** | 依個人語言清單翻譯，結果僅自己可見。若尚未設定語言，會顯示提示說明如何設定 |
| **查看原文** | 查看該翻譯訊息的原始文字、來源頻道與發送者 |
| **重新翻譯** | 強制重新翻譯並更新訊息內容（bypass cache） |
...
| 右鍵 → **翻譯圖片文字** | 對含圖片的訊息執行 OCR 辨識文字，翻譯後**直接畫回圖片覆蓋原文字**，並廣播到同群組所有語言頻道；若覆蓋失敗則自動退化為純文字翻譯訊息（僅操作者收到處理結果回報） |
...
| 限制 | 說明 |
|------|------|
| 追蹤記憶體持久化 | 容器重啟後自動還原追蹤記錄，最多保留最近 **1,500** 則訊息（可透過 `.env` 的 `MAX_CLUSTER_ENTRIES` 調整）；超過上限的舊訊息將被淘汰 |
...
| 圖片翻譯為「盡力而為」 | OCR 辨識與覆蓋繪製效果因圖片而異；複雜排版、特殊字體、透明背景或行數判斷失準時，會自動退化為純文字翻譯（不覆蓋圖片）。OCR 以頻道設定語言作為辨識提示，若圖片實際語言與頻道語言不同會降低辨識率 |
